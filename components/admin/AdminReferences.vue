...
const defaultForm = () => ({ name: '', title: '', company: '', email: '', is_visible: true, sort_order: 0 })
const form = ref(defaultForm())

async function load() {
  loading.value = true
  try {
    items.value = await apiFetch<any[]>('/api/references')
  } catch { /* ignore */ }
  loading.value = false
}
...
async function save() {
  try {
    if (editing.value) {
      await apiFetch(`/api/references/${editing.value.id}`, { method: 'PUT', body: form.value })
    } else {
      await apiFetch('/api/references', { method: 'POST', body: form.value })
    }
...
async function toggleVisible(id: number) {
  const updated = await apiFetch(`/api/references/${id}/toggle-visible`, { method: 'PATCH' })
...
<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-4">
      <button @click="openAdd" class="btn-primary text-sm">+ Add Reference</button>
    ...
    <div v-else class="space-y-3">
      <div v-for="item in items" :key="item.id" class="card flex items-start justify-between gap-4">
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-text-muted font-tech">{{ item.title }} at {{ item.company }}</p>
          <p class="text-xs text-text-muted font-tech">{{ item.email }}</p>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showModal = false">
        ...
        <div class="relative bg-bg-secondary rounded-lg border w-full max-w-lg p-6" style="border-color: rgb(var(--color-border) / 0.2)">
          ...
          <form @submit.prevent="save" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="block text-xs font-tech text-text-secondary uppercase tracking-wider mb-1">Title</label>
                <input v-model="form.title" required class="w-full rounded border bg-bg-secondary text-text-primary placeholder-text-muted/50 px-4 py-2.5 text-sm focus:outline-none focus:border-accent/60 transition-colors" style="border-color: rgb(var(--color-border) / 0.2)" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input v-model="form.email" type="email" required class="w-full rounded border bg-bg-secondary text-text-primary placeholder-text-muted/50 px-4 py-2.5 text-sm focus:outline-none focus:border-accent/60 transition-colors" style="border-color: rgb(var(--color-border) / 0.2)" />
              </div>
            </div>
