...
async function openEdit(e: any) {
  ...
  try {
    ...
    form.value = { ...data, bullets: data.bullets || [], tags: data.tags || [] }
    showModal.value = true
  } catch { /* ignore */ }
}
    ...
    <div v-else class="space-y-3">
      <div v-for="e in experiences" :key="e.id" class="card flex items-start justify-between gap-4">
        ...
        <div class="flex items-center gap-2 shrink-0">
          <button @click="toggleVisible(e.id)" class="btn-ghost text-xs">{{ e.is_visible ? 'Hide' : 'Show' }}</button>
          <button @click="openEdit(e)" class="btn-ghost text-xs">Edit</button>
          <button @click="remove(e.id)" class="text-xs text-red-400 hover:text-red-300 font-tech">Delete</button>
        </div>
      </div>
    </div>
